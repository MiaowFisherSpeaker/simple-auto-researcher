...
metadata:
  {
    "openclaw":
      {
        "emoji": "🔬",
        "requires": { "tools": ["web_search", "web_fetch", "exec"] },
      },
  }
---
...
**所有任务必须在 `~/Documents/AutoResearcher/` 目录下执行！**
...
| 创新点  | AutoResearcher-创新点 | 10 分钟 | 1 个          |
...
2. 按照上述工作量安排，如果已下载但未阅读的论文数量不足 `“工作量”` 篇，则需要额外搜索补充新的论文，确保本次搜索能产出 **`“工作量”` 篇新的待下载论文**，否则可以跳过本次搜索任务，并做好日志记录。
...
2. 如果未归档论文数 ≥ `“工作量”` 篇，执行后续归档步骤；否则跳过本次任务并记录日志
...
2. **保存到** `idea/all/*.md`（按字母+6位数字命名）
   - 创新点文档命名规范：
     ...
     - 如果文档里已经有idea文件了，计数要从目录中最大的数往后计算，比如a...11,下一个生成的就应该是a...12,如果是b999999,那么应该有能力且应该推算出下一个是c000001，后面以此类推
...
- 禁止通过模型修改该skill的README.md的内容，只允许用户自己修改。
