...
- 阅读 `PREFERENCES.md` 了解用户的“联网搜索偏好”、“偏好会议或者期刊”等
...
| 任务 ID | 任务名称              | 间隔    | 每次工作量 |
| ------- | --------------------- | ------- | ---------- |
| 搜索    | AutoResearcher-搜索   | 10 分钟 | 5 篇       |
| 下载    | AutoResearcher-下载   | 10 分钟 | 5 篇       |
| 阅读    | AutoResearcher-阅读   | 10 分钟 | 5 篇       |
...
2. 遍历 `files/downloaded/`每个日期目录，看论文数（pdf数量）与notes目录下对应日期的md文件数量是否一致。优先遍历日期最近的目录。
   - 如果不一致，说明有未阅读的论文，收集5篇未阅读的论文信息，继续执行后续步骤
   - 如果一致，说明没有未阅读的论文，可以跳过本次阅读任务，continue到下一个日期目录。
3. 从刚刚获得的选择 **5 篇**未阅读的论文（优先最新日期），
   - 如果不足 5 篇，则全部选取。但是如果发现其他日期还有未阅读的论文，则继续选取，直到选满 5 篇或者没有更多未阅读的论文为止；
   - 如果足够5篇，检查一下其他日期目录是否还有未阅读的论文，如果有，则多阅读1篇，即阅读的工作量+1篇。
